...
def test_new_task_group():
    ...
    tg = TaskGroup('new_task', [t1, t2], 'context', 'file1.ceryle')

    assert tg.name == 'new_task'
    assert tg.tasks == [t1, t2]
    assert tg.context == 'context'
    assert tg.filename == 'file1.ceryle'


def test_new_task_group_deps():
    tg = TaskGroup('build', [], 'context', 'file1.ceryle', dependencies=['setup', 'pre-build'])
...
def test_run_tasks(mocker):
    ...
    t1 = Task(Command('do some'))
    t2 = Task(Command('do some'))
    tg = TaskGroup('tg', [t1, t2], 'context', 'file1.ceryle')
    ...
    assert mock.mock_calls == [
        ...
        mocker.call.g1_t2_run('context', dry_run=False, inputs=[]),
    ]
...
def test_run_task_set_stderr_to_register(mocker):
    t1 = Task(Command('do some'), stderr='EXEC_STDERR')
...
def test_run_task_with_input_from_register(mocker):
    t1 = Task(Command('do some'), input=('another_tg', 'EXEC_STDOUT'))
    mocker.patch.object(t1, 'stdout', return_value=[])
    mocker.patch.object(t1, 'stderr', return_value=[])
    mocker.patch.object(t1, 'run', return_value=True)
    tg = TaskGroup('tg', [t1], 'context', 'file1.ceryle')
    ...
    t1.run.assert_called_once_with('context', dry_run=False, inputs=['foo', 'bar'])
    t1.stdout.assert_not_called()
    t1.stderr.assert_not_called()
...
def test_run_task_with_input_from_same_group(mocker):
    ...
    t2 = Task(Command('do some'), input='EXEC_STDOUT')
    ...
    t2.run.assert_called_once_with('context', dry_run=False, inputs=['foo', 'bar'])
    t2.stdout.assert_not_called()
    t2.stderr.assert_not_called()
    t1.run.assert_called_once_with('context', dry_run=False, inputs=[])
    t1.stdout.assert_called_once()
    t1.stderr.assert_not_called()


def test_run_failed_by_invalid_io(mocker):
    t1 = Task(Command('do some'), input='EXEC_STDOUT')
    mocker.patch.object(t1, 'run', return_value=True)
    tg = TaskGroup('tg', [t1], 'context', 'file1.ceryle')
